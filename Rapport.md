...
Le patron de conception proposé par cette méthode pour parcourir tous les objets d'un type donné disponibles dans le contexte applicatif est le patron de conception Iterator. En effet, cette méthode renvoie un objet de type Iterator pour parcourir les éléments d’une collection sans connaître son implémentation.

En vue de modifier la simulation de vélo pour utiliser cette même méthode afin de simuler tous les types de vélo presénts dans la configuration, nous avons premièrement complété le code de la fonction injectAll() de la classe Context en utilisant ServiceLoader.load(klass) pour charger toutes les implémentations de la classe abstraite Bike, c'est-à-dire les instances des classes SimpleBike et TagAlongBike.
Par la suite, nous avons modifié la méthode run() présente dans la classe BikeSimulator. En effet, nous avons fait usage de la fonction injectAll() afin d'itérer sur toutes les instances de la classe Bike. Cela nous a permis de simuler chaque type de vélo disponible dans le fichier META-INF.services/fr.polytech.sim.cycling.Bike.

Voici le résultat obtenu dans notre fichier logs.txt : 

![image](https://github.com/PolytechLyon/2024-3a-isi3-tp6-g2-bala_magassa_tp6/assets/160489283/df82c304-3d7a-4e5d-ad32-7f60af01f3a0)
